frappe.pages['palcare-counsellor'].on_page_load = function (wrapper) {
	...
	let feed=$(frappe.render_template("couns_data")).appendTo(page.main);;
	let couns_pal = frappe.render_template("couns_pal");
	let route = window.location.hash
	let coun_url=window.location.href
	let decode_url=decodeURIComponent(coun_url)
	...
	let Docter1 = document.getElementById("docter1")
	...
	//Patient Against redirection to docters note and nurse note page
	frappe.db.get_doc('Patient', null, { name: split_hash }).then(rec => {
		Docter1.onchange = function (e) {
			if (document.getElementById('docter1').value == "palcare-note_display") {
				frappe.set_route(`/app/palcare-note_display/#${rec.name}`)
				window.location.reload()

			}
			else if (document.getElementById('docter1').value == "palcare-notes") {
	...
	//Patient Against redirection to docters note and nurse note page end
	...
	frappe.db.get_list('Notes', {
		...
		filters: {
			...
			note_type: 'Counsellor'
		},limit:100,
	}).then(records => {
		if (records.length > 0) {
			records.forEach(patient => {
				var new_patient = $(couns_pal);
				new_patient.find('.counseller_note').html(patient.c_note);
				...
				$(".notes3").append(new_patient);

				//Removing Edit button if there is no data
				$(document).ready(function () {
					let isnotes3 = document.getElementsByClassName('notes3');
					if (isnotes3.length > 0) {
			...
			$(".notes3").append(`<h3 style=text-align:center;>Nothing to show</h3>`)
		};
	})
	...
	$("body").on('click', '.dcom', function (e) {
		...
		frappe.set_route("List", "Notes", { "patient_name": `${split_hash}`, "note_type": "Counsellor" })
