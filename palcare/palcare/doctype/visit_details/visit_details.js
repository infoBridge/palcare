...
let dob = new Date(),
	today = new Date(),
	calTime;
frappe.ui.form.on('Visit Details', {
	after_save: function (frm) {
		//Redirecting to Patient Details page After saving
		frappe.set_route(`/app/patient-details/#${frm.doc.patient}`)
	...
	onload: function (frm) {
		...
		$("[data-fieldname='visit_date']").on('change', 'input', function (e) {
			function calculate(input) {
				...
				frappe.db.get_list('Patient', {
					...
					records.forEach(med => {
						...
						if (com == med.name) {
							...
							let coms1 = new Date(cur_frm.doc.visit_date)
							...
							if (year < 0) {
								alert("Visit Date Can't be Before Date Of Enrolment:-" + result)
								if ($("[data-fieldname='visit_date']").find('input').val()) {
									$("[data-fieldname='visit_date']").val('n').trigger('change')
				...
				if (year < 0) {
					alert("Please Enter a valid visit Date")
					if ($("[data-fieldname='visit_date']").find('input').val()) {
						$("[data-fieldname='visit_date']").val('n').trigger('change')
						$('.modal-backdrop.fade.show').hide();
						$('.modal').remove();
			...
			calculate($("[data-fieldname='visit_date']").find('input').val())
		})
				...
				frappe.db.get_list('Patient', {
					...
					records.forEach(med => {
						...
						if (com == med.name) {
							...
							if (year < 0) {
								alert("Date Can't be Before Date Of Enrolment ")
