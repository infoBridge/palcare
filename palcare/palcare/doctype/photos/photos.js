...
frappe.ui.form.on('Photos', {
	...
	onload: function (frm) {
		...
		$("[data-fieldname='uploaded_on']").on('change', 'input', function (e) {
				...
				frappe.db.get_list('Patient', {
					...
					records.forEach(med => {
						...
						if (com == med.name) {
							...
							let coms1 = new Date(cur_frm.doc.uploaded_on)
							...
							if (year < 0) {
								...
								if ($("[data-fieldname='uploaded_on']").find('input').val()) {
									$("[data-fieldname='uploaded_on']").val('n').trigger('change')
			...
			calculate($("[data-fieldname='uploaded_on']").find('input').val())
		})
